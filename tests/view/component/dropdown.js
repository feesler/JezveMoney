...
export class DropDown extends NullableComponent
{
    ...
    async parse()
    {
        ...
        if (this.isMulti)
        {
            ...
            {
                let deselectBtn = await this.query(el, '.dd__del-selection-item-btn');

                let title = await this.prop(el, 'textContent');
                let ind = title.indexOf('×');
                if (ind !== -1)
                    title = title.substr(0, ind);
                
                let id = await this.prop(el, 'dataset.id');
    ...
    getSelectedItem(item_id)
    {
        return this.selectedItems.find(item => item.id == item_id);
    }
    ...
    async deselectItemByTag(item_id)
    {
        if (!this.isMulti)
            throw new Error('Deselect item not available for single select DropDown');

        let selItem = this.getSelectedItem(item_id);
        if (!selItem)
            throw new Error(`Selected item ${item_id} not found`);
