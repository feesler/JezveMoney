...
function createExpense(view, accNum, onState, params)
{
	return App.goToMainView(view)
			.then(view => view.goToNewTransactionByAccount(accNum))
			.then(view => expenseTransactionLoop(view, onState, view => submitExpenseTransaction(view, params)))
			.then(async view =>
			{
...
function updateExpense(view, pos, params)
{
	pos = parseInt(pos);
	if (isNaN(pos) || pos < 0)
	...
	return App.goToMainView(view)
			...
			.then(async view =>
			{
				...
				return submitExpenseTransaction(view, params);
			})
			.then(view => view.filterByType(EXPENSE))
			.then(async view =>
			{
